"""
Phase 1 (NO-DTW ABLATION): Compute lockstep top-k neighbor indices on
proprioceptive trajectories.

This is the DTW-ablation counterpart of compute_dtw.py. Everything is held
identical -- the same proprio trajectories ({data_dir}/feats_B_seq.pt), the
same per-step pose cost (pos L2 + quat geodesic + grip L1, uniform weights
(1,1,1)), the same horizon slicing [:, 1:h+1, :], the same valid-task mask,
the same top-k=10. The ONLY change: instead of finding the optimal DTW
warping path, trajectories are aligned lockstep (frame t to frame t), i.e.

    dist_lockstep(i, j) = sum_t cost(traj_i[t], traj_j[t])

which is the diagonal of the (T x T) per-step cost matrix. This isolates the
contribution of temporal warping: with-DTW vs no-DTW differ only in alignment.

Outputs:
    - {output_dir}/dtw_topk_h{H}_k{K}_lockstep_nowin_nopad.npy  (one per horizon)
      (filename keeps the dtw_topk_ prefix so Phase 2 / compute_cknna_*.py can
       read it unchanged via --dtw_version lockstep_nowin)

Usage:
    conda run -n starVLA python compute/compute_nodtw.py \
        --data_dir /lambda/nfs/vla/cache/cknna_data_store/cknna_data_droid \
        --output_dir /lambda/nfs/vla/cache/cknna_data_store/record/dtw_cache/droid_nodtw \
        --horizons 7 \
        --topk 10
"""
...
TOPK = 10
WEIGHTS = (1.0, 1.0, 1.0)  # uniform: pos, rot, grip -- identical to compute_dtw.py


# -- Padding detection (identical to compute_dtw.py) --------------------------
...
# -- Per-step pose cost (identical to compute_dtw.py) -------------------------

def _build_pose_cost_matrices_gpu(pos_i, grip_i, quats_i,
                                  all_pos, all_grip, all_quats, weights):
    """D[b, t_i, t_j] = weighted pose cost between traj_i[t_i] and traj_b[t_j]."""
...
def compute_lockstep_topk_gpu(trajs_np, topk):
    """Compute pairwise lockstep (no-warp) top-k on GPU.

    Lockstep distance = sum over t of the per-step cost on the t==t diagonal.
    No DP, no warping path -- the only difference from compute_dtw.py.
    """
    ...
    with torch.no_grad():
        for i in range(N):
            ...
            # Lockstep: sum the t==t diagonal instead of running DTW DP.
            distances = D.diagonal(dim1=1, dim2=2).sum(dim=1)  # [N]
...
def compute_lockstep_topk_h1(trajs_np, topk):
    """Fast path for h=1: lockstep on length-1 == pairwise pose distance
    (identical to DTW at h=1 -- no warping is possible for a single step)."""
    w_pos, w_rot, w_grip = WEIGHTS
    N = len(trajs_np)
...
# -- Main (identical structure to compute_dtw.py) -----------------------------

def main():
    parser = argparse.ArgumentParser(
        description="Phase 1 (NO-DTW ablation): lockstep top-k on proprio trajectories.")
    parser.add_argument("--data_dir", required=True,
                        help="Dir with feats_B_seq.pt and metadata.json")
    parser.add_argument("--output_dir", required=True,
                        help="Where to save dtw_topk_*_lockstep_*.npy cache files")
    ...
    for h in args.horizons:
        out_path = os.path.join(
            args.output_dir,
            f"dtw_topk_h{h}_k{args.topk}_lockstep_nowin_nopad.npy")
        ...
        if h == 1:
            topk = compute_lockstep_topk_h1(traj, args.topk)
        else:
            topk = compute_lockstep_topk_gpu(traj, args.topk)
