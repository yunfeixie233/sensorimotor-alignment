...
def convert_dataset(
    ...
    if list(root.glob("*.conversion_completed.json")):
        logging.info("Conversion completion marker found; skipping conversion for %s", root)
        return
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--repo-id",
        type=str,
        default="lerobot/agibotworld",
        help="Repository identifier on Hugging Face (e.g. `lerobot/pusht`).",
    )
    parser.add_argument(
        "--root",
        type=str,
        default="/mnt/xlab-nas-2/vla_dataset/lerobot/agibot_convert_incomplete/agibotworld/task_354",
