...
def convert_info(
    ...
    source_total = info.get("total_episodes")
    total_episodes = len(episode_records)
    if source_total is not None and source_total != total_episodes:
        logging.warning(
            "Input info.json total_episodes=%s does not match meta/episodes=%s; using meta/episodes",
            source_total,
            total_episodes,
        )
...
def _get_video_fps(video_path: Path) -> float:
    """Get video FPS from video file or fallback to default."""
    try:
        ...
        logging.warning(f"Could not get FPS from video {video_path}: {e}, using default 30.0")
        return 30.0
...
def _extract_video_segment(
    ...
    """Extract a video segment using ffmpeg based on frame numbers.
    ...
    Args:
        ...
        fps: Video FPS. If None, will be read from video file.
    """
    ...
    # if fps is None:
    #     fps = _get_video_fps(src)
    fps = _get_video_fps(src)
    
    # Convert timestamps to frame indices
    # Round to nearest frame for accuracy (frame indices start from 0)
    start_frame = int(round(start * fps))
    end_frame = int(round(end * fps))
    num_frames = max(1, end_frame - start_frame)

    if num_frames > fps * 3600:  # More than 1 hour at this FPS
        raise ValueError(f"Video segment too long: {num_frames} frames at {fps} fps")

    dst.parent.mkdir(parents=True, exist_ok=True)

    # Calculate start time in seconds for fast seeking
    start_time = start 
    
    # Use -ss for fast seeking to start time, then extract exact number of frames
    # This is faster than using select filter which processes all frames
    cmd = [
        "ffmpeg",
        "-hide_banner",
        "-loglevel",
        "error",
        "-ss",
        f"{start_time:.6f}",  # Fast seek to start time
        "-i",
        str(src),
        "-frames:v",
        str(num_frames),  # Extract exactly num_frames frames
        "-vsync",
        "0",  # Disable frame rate conversion to preserve original frame timing
        "-c:v",
        "libx264",
        "-preset",
        "fast",
...
def _get_video_fps_from_info(root: Path, video_key: str) -> float | None:
    """Get video FPS from info.json if available."""
    try:
        info = load_info(root)
        features = info.get("features", {})
        if video_key in features:
            video_feature = features[video_key]
            # Try different possible locations for FPS
            if "video_info" in video_feature:
                fps = video_feature["video_info"].get("video.fps")
                if fps is not None:
                    return float(fps)
            if "info" in video_feature:
                fps = video_feature["info"].get("video.fps")
                if fps is not None:
                    return float(fps)
    except Exception as e:
        logging.debug(f"Could not get FPS from info.json for {video_key}: {e}")
    return None


def convert_videos(root: Path, new_root: Path, episode_records: list[dict[str, Any]], video_keys: list[str]) -> None:
    """Convert concatenated MP4 files back to per-episode videos using frame-based extraction."""
    if len(video_keys) == 0:
        logging.info("No video features detected; skipping video conversion")
        return

    logging.info("Converting concatenated MP4 files back to per-episode videos (using frame-based extraction)")

    for video_key in video_keys:
        # Try to get FPS from info.json first
        fps_from_info = _get_video_fps_from_info(root, video_key)
        ...
        for (chunk_idx, file_idx), records in tqdm.tqdm(grouped.items(), desc=f"convert videos ({video_key})"):
            ...
            # Get FPS from video file (will use fps_from_info if available, otherwise read from file)
            # We read it once per video file to avoid repeated reads
            # video_fps = fps_from_info
            # if video_fps is None:
            #     video_fps = _get_video_fps(src_path)
            video_fps = _get_video_fps(src_path)
            ...
            for record in records:
                ...
                _extract_video_segment(src_path, dest_path, start=start, end=end, fps=video_fps)


def reconcile_info_with_disk(new_root: Path, video_keys: list[str]) -> None:
    """Recompute info.json totals from on-disk episode files."""
    info = load_info(new_root)
    episode_files = list(new_root.glob("data/chunk-*/episode_*.parquet"))
    actual_episodes = len(episode_files)
    chunks_size = info.get("chunks_size", DEFAULT_CHUNK_SIZE)

    info["total_episodes"] = actual_episodes
    info["total_chunks"] = math.ceil(actual_episodes / chunks_size) if actual_episodes > 0 else 0
    info["total_videos"] = actual_episodes * len(video_keys)

    write_info(info, new_root)
    logging.info("Updated info.json with on-disk episode count=%s", actual_episodes)
...
def convert_dataset(input_path: Path, output_path: Path, force: bool = False) -> None:
    ...
    reconcile_info_with_disk(output_path, video_keys)
...
def main():
    ...
    parser.add_argument(
        "--task-id",
        type=str,
        default="241223_upright_cup",
        help="Task ID (e.g., task_351). If provided, will be used to construct input and output paths.",
    )
    parser.add_argument(
        "--input-path",
        type=Path,
        default=Path("/mnt/xlab-nas-2/vla_dataset/lerobot/robomind/benchmark1_2_compressed/franka_3rgb/"),
        help="Path to the v3.0 format dataset folder",
    )
    parser.add_argument(
        "--output-path",
        type=Path,
        default=Path("/mnt/xlab-nas-2/vla_dataset/lerobot/robomind/lerobot_v21"),
    ...
    output_path = output_base_path / subname
