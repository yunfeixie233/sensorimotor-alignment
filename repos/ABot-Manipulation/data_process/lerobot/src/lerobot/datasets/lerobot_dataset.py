    ...
    def get_video_file_path(self, ep_index: int, vid_key: str) -> Path:
        ...
        try:
            if f"videos/{vid_key}/chunk_index" in ep:
                ...
                fpath = self.video_path.format(video_key=vid_key, episode_chunk=chunk_idx, episode_index=episode_index)
            
        except:
            fpath = self.video_path.format(video_key=vid_key, episode_chunk=0, episode_index=ep['episode_index'])
        return Path(fpath)
    ...
    def get_task_index(self, task: str) -> int | None:
        ...
        try:
            ...
            try:
                ...
                self.load_metadata()
            task_index = self.tasks.index
        if task in task_index: #self.tasks.index:
...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def load_hf_dataset(self) -> datasets.Dataset:
        ...
        selected_parquet_paths = None
        if self.episodes is not None:
            try:
                selected_parquet_paths = sorted({self.root / self.meta.get_data_file_path(ep_idx) for ep_idx in self.episodes})
            except (KeyError, IndexError, ValueError, TypeError):
                # Fallback for legacy datasets where parquet files follow:
                # data/chunk-xxx/episode_yyyyyy.parquet
                legacy_paths = []
                for ep_idx in self.episodes:
                    chunk_idx = ep_idx // DEFAULT_CHUNK_SIZE
                    legacy_path = self.root / f"data/chunk-{chunk_idx:03d}/episode_{ep_idx:06d}.parquet"
                    if legacy_path.exists():
                        legacy_paths.append(legacy_path)
                selected_parquet_paths = sorted(set(legacy_paths)) if len(legacy_paths) > 0 else None
        hf_dataset = load_nested_dataset(
            self.root / "data", features=features, episodes=self.episodes, parquet_paths=selected_parquet_paths
        )
    ...
    def _query_videos(self, query_timestamps: dict[str, list[float]], ep_idx: int) -> dict[str, torch.Tensor]:
        ...
        for vid_key, query_ts in query_timestamps.items():
            ...
            try:
                ...
                shifted_query_ts = [0 + ts for ts in query_ts]
