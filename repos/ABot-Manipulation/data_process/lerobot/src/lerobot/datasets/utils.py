...
def _episode_index_in_set(example: dict, *, episode_set: frozenset) -> bool:
    """Module-level predicate for Dataset.filter (pickle-friendly; avoids lambda fingerprint warnings)."""
    ei = example["episode_index"]
    if hasattr(ei, "item"):
        ei = ei.item()
    return ei in episode_set
...
def load_nested_dataset(
    ...
    """Load episode metadata from either:
        - Multiple parquet files in subdirectories (original LeRobot format), OR
        - A single 'episodes.jsonl' file (new compact format).

    Args:
        pq_dir: Directory that either contains parquet shards or 'episodes.jsonl'
        features: Optional HF Features schema (required for consistent types, e.g., List[str])
        episodes: Optional list of episode indices to filter (uses predicate pushdown for parquet;
                  manual filtering for JSONL)

    Returns:
        datasets.Dataset: Unified HF Dataset object
    """
    
    # Case 1: JSONL exists → use it (higher priority if both exist)
    # print("xxxxxxxxx",pq_dir)
    if pq_dir.suffix == ".jsonl":
        # Read JSONL into list of dicts
        records = []
        with open(pq_dir, "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if line:
        ...
        if episodes is not None:
            ...
            records = [r for r in records if r.get("episode_index") in episode_set]

        # Convert to Dataset
        df = Dataset.from_list(records)

        # Cast to provided features if needed
        if features is not None:
            # Convert to Arrow Table, cast, then back to Dataset
            table = df.data  # PyArrow Table
            table = table.cast(features.arrow_schema)
            df = Dataset(table)

        return df

    # Case 2: Parquet shards exist
    else:
        parquet_paths = sorted(parquet_paths) if parquet_paths is not None else sorted(pq_dir.glob("*/*.parquet"))
        with SuppressProgressBars():
            # Fast path when we don't need episode filtering: rely on HF/Arrow defaults.
            if episodes is None:
                return Dataset.from_parquet([str(p) for p in parquet_paths], features=features)

            # When `episodes` is provided, avoid using PyArrow `isin` on columns that may be
            # stored as `list<int64>` in some datasets, which can trigger
            # `ArrowNotImplementedError: Function 'is_in' has no kernel matching input types`.
            # Instead, load the full dataset and filter at the HF Dataset level.
            ds = Dataset.from_parquet([str(p) for p in parquet_paths], features=features)
            episode_set = frozenset(episodes)
            if "episode_index" in ds.column_names:
                ds = ds.filter(partial(_episode_index_in_set, episode_set=episode_set))

            # Do not use `table = ds.data.cast(...); ds = Dataset(table)` after `filter()`:
            # `ds.data` is the full underlying Arrow table (e.g. 65204 rows), not the filtered view
            # (e.g. 470 rows), so rebuilding from `table` drops the episode filter.
            if features is not None:
                try:
                    ds = ds.cast(features)
                except Exception:
                    # Fallback to the uncast dataset if casting fails for any reason.
                    pass
    ...
    #         f"Neither 'episodes.jsonl' nor parquet shards found in: {pq_dir}"
    #     )
...
def load_episodes(local_dir: Path) -> datasets.Dataset:
    ...
    parquet_path = local_dir / EPISODES_DIR
    jsonl_path = local_dir / "meta/episodes.jsonl"

    if parquet_path.exists():
        episodes = load_nested_dataset(parquet_path)
    elif jsonl_path.exists():
        episodes = load_nested_dataset(jsonl_path)
