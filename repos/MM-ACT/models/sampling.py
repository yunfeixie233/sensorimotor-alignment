...
def mask_by_random_topk(mask_len, probs, temperature=1.0, generator=None):
    ...
    return masking
def mask_by_random_topk_action(mask_len, scores, temperature=1.0, generator=None):
    B, L = scores.shape
    order = scores.argsort(dim=1)                              # [B, L]
    ranks = torch.empty_like(order)                            # [B, L]
    ranks.scatter_(1, order, torch.arange(L, device=scores.device).unsqueeze(0).expand(B, L))
    k = mask_len.clamp(min=1, max=L).to(scores.device).long()  # [B]
    masking = ranks < k.unsqueeze(1)                           # [B, L]
    return masking
