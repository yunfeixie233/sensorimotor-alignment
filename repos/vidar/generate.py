...
def generate(args):
    ...
    for batch_idx, batch_data in enumerate(dataloader):
        ...
        if media_path:
            ...
            # logging.info(f"Input image: {media_path}")

        # prompt extend
        current_prompt = prompt
        if args.use_prompt_extend and prompt_expander:
            ...
            if rank == 0:
                ...
                input_prompt_list = [None]
            if dist.is_initialized():
                dist.broadcast_object_list(input_prompt_list, src=0)
            current_prompt = input_prompt_list[0]
            logging.info(f"Extended prompt: {current_prompt}")
        ...
        if args.ulysses_size == 1 or rank == 0:
            if args.save_file and not args.dataset_json:
                ...
                if file_stem:
                    ...
                    formatted_prompt = current_prompt.replace(" ", "_").replace("/",
                                                                            "_")[:50]
                    suffix = '.mp4'
