...
from distributed_utils import init_distributed_device, world_info_from_env
...
def evaluate_policy(
    ...
    conf_dir = Path("/root/calvin/calvin_models") / "conf"
    ...
    with open("/mnt/cpfs_m6_2b4fd84a/jianke/jianke_z/VLM4VLA/configs/data/calvin/eval_sequences.json", "r") as f:
...
def evaluate_policy_ddp(
    ...
    for initial_state, eval_sequence in eval_sequences:
        ...
        with open(eval_sr_path, "a") as f:
            ...
            line += "\n"
            f.write(line)
        eval_sequences.set_description(" ".join([f"{i + 1}/5 : {v * 100:.1f}% |" for i, v in enumerate(success_list)]) +
                                       "|")
...
def rollout(
    ...
    for _ in range(EP_LEN):
        action = model.step(obs, lang_annotation, execute_step=10)
        ...
        if len(current_task_info) > 0:
            if debug:
                ...
                clip.write_gif(
                    os.path.join(eval_log_dir, f"{sequence_i}-{subtask_i}-{subtask}-succ.gif"),
                    fps=30,
                )
            return True
...
def parser_args():
    ...
    parser.add_argument(
        "--dataset_path",
        default="/home/disk1/jianke_z/task_ABC_D",
        type=str,
...
def main():
    ...
    # new_dir = "/mnt/cpfs_m6_2b4fd84a/jianke/jianke_z"
    # old_dir = "/mnt/zjk/jianke_z"
    # keys_to_change = [
    #     "model_path", "model_config", ["tokenizer", "pretrained_model_name_or_path"],
    #     ["vlm", "pretrained_model_name_or_path"], ["train_dataset", "data_dir"], ["val_dataset", "data_dir"]
    # ]
    ...
    #                 new_path = "/mnt/cpfs_m6_2b4fd84a/jianke/jianke_z/huggingface/hub/models--Qwen--Qwen2.5-VL-3B-Instruct/snapshots/c747f21f03e7d0792c30766310bd7d8de17eeeb3"
    #             elif "7b" in path or "7B" in path:
    #                 new_path = "/mnt/cpfs_m6_2b4fd84a/jianke/jianke_z/huggingface/hub/models--Qwen--Qwen2.5-VL-7B-Instruct/snapshots/cc594898137f460bfe9f0759e9844b3ce807cfb5"
    #             else:
    ...
    #                 new_path = "/mnt/cpfs_m6_2b4fd84a/jianke/jianke_z/VLMA-baselines/1/qwen3vl-4b"
    #             elif "8b" in path or "8B" in path:
    #                 new_path = "/mnt/cpfs_m6_2b4fd84a/jianke/jianke_z/VLMA-baselines/1/qwen3vl-8b"
    #             elif "2b" in path or "2B" in path:
    #                 new_path = "/mnt/cpfs_m6_2b4fd84a/jianke/jianke_z/VLMA-baselines/1/qwen3vl-2b-instruct"
    #             else:
    ...
    if args.is_pt_config:
        ...
        from vlm4vla.utils.config_utils import generate_calvin_ft_configs, deep_update
    ...
    gpus_per_process= torch.cuda.device_count() // args.world_size
    print(f"gpus per rank: {gpus_per_process}")
    start_gpu_index = args.rank * gpus_per_process
    end_gpu_index = start_gpu_index + gpus_per_process
    device_ids = list(range(start_gpu_index, end_gpu_index))

    # torch.cuda.set_device(device_ids)
    os.environ['CUDA_VISIBLE_DEVICES'] = ",".join(map(str, device_ids))
    print(f"local rank: {args.local_rank} | rank: {args.rank} | world size: {args.world_size}: Using gpu {device_ids}")
    ...
    print("vlm",model.model.model.model.device)
    # print("act_head",model.model.model.act_head.device)
    # model.model.model.act_head=model.model.model.act_head.to(model.model.model.model.device)
    print(configs["val_dataset"])
    ...
    result_path = os.path.join(eval_log_dir, f"results_calvin_{ckpt_step}{suffix}_rand-{args.rank}.json")
