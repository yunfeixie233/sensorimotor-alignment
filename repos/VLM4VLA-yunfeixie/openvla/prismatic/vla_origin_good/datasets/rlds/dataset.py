...
def make_dataset_from_rlds(
    ...
    *,
    ...
    def restructure(traj):
            ...
            new_obs["proprio"] = tf.concat(
                [
                    (
                        ...
                        if key is None else tf.cast(old_obs[key], tf.float32)) for key in state_obs_keys
                ],
                axis=1,
            )
        ...
        if language_key is not None:
            if traj[language_key].dtype != tf.string:
                raise ValueError(f"Language key {language_key} has dtype {traj[language_key].dtype}, "
        ...
        if absolute_action_mask is not None:
            if len(absolute_action_mask) != traj["action"].shape[-1]:
                raise ValueError(f"Length of absolute_action_mask ({len(absolute_action_mask)}) "
    ...
    # print("here! name", name)  # name: bridge_orig
    # print("here! data_dir", data_dir)  # data_dir: /mnt/zjk/jianke_z/open-x-embodiment
    ...
    if isinstance(dataset_statistics, str):
        ...
        full_dataset = dl.DLataset.from_rlds(
            ...
            num_parallel_reads=num_parallel_reads).traj_map(restructure, num_parallel_calls)
    ...
    if action_normalization_mask is not None:
        if len(action_normalization_mask) != dataset_statistics["action"]["mean"].shape[-1]:
            raise ValueError(f"Length of skip_normalization_mask ({len(action_normalization_mask)}) "
...
def apply_trajectory_transforms(dataset: dl.DLataset,
                                *,
                                train: bool,
    ...
    if chunk_action:
        ...
        dataset = dataset.traj_map(
            partial(traj_transforms.chunk_as_episode, frame_num=frame_num),
            num_parallel_calls,
        )
...
# === Core Initializer ===
def make_interleaved_dataset(dataset_kwargs_list: List[Dict],
