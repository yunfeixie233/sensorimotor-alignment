...
from robo_orchard_lab.models.mapdream.janus.inference.utils import conversation
...
class VLChatProcessor(ProcessorMixin):
    ...
    def apply_sft_template_for_multi_turn_prompts(
        ...
        Args:
            conversations (List[Dict]): A conversation with a List of
                Dict[str, str] text.
            sft_format (str, optional): The format of the SFT template
                to use. Defaults to "deepseek".
            system_prompt (str, optional): The system prompt to use in
                the SFT template. Defaults to "".
    ...
    def add_image_token(
        ...
        """Insert image tokens into a tokenized prompt.
        ...
        for index in image_indices:
            ...
            input_slices.append(
                self.image_start_id * torch.ones((1), dtype=torch.long)
            )
            input_slices.append(
                self.image_id
                * torch.ones(
            ...
            input_slices.append(
                self.image_end_id * torch.ones((1), dtype=torch.long)
            )
            start = index + 1
    ...
    def process_one(
        ...
        **kwargs,
        ...
        """Process a single prompt or conversation.
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """Process inputs and optionally batchify them.
    ...
    def batchify(
        ...
        Args:
            prepare_list (List[VLChatProcessorOutput]): A list of
                VLChatProcessorOutput.

        Returns:
            BatchedVLChatProcessorOutput: A dictionary of the inputs to
                use for multimodal inference.
        """
        ...
        batched_attention_mask = torch.zeros(
            (batch_size, input_token_max_len)
        ).long()
