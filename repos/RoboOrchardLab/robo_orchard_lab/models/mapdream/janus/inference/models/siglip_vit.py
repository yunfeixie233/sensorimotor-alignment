...
def trunc_normal_(tensor, mean=0.0, std=1.0, a=-2.0, b=2.0):
    ...
    r"""Truncated normal initialization supporting bfloat16.

    The original timm implementation cannot handle bfloat16, so we first
    convert the tensor to float32, apply trunc_normal_(), and then cast
    it back to the original dtype.
    ...
    distribution :math:`\mathcal{N}(\text{mean}, \text{std}^2)` with
    values outside :math:`[a, b]` redrawn until they are within the bounds.
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        # or partial(nn.LayerNorm, eps=1e-6)
    ...
    def init_weights(
        self,
        mode: Literal["jax", "jax_nlhb", "moco", ""] = "",
    ) -> None:
    ...
    def reset_classifier(self, num_classes: int, global_pool=None) -> None:
        ...
        if global_pool is not None:
            ...
            if global_pool == "map" and self.attn_pool is None:
                raise AssertionError("Cannot currently add attention pooling.")
    ...
    def _pos_embed(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.no_embed_class:
            ...
            # Position embedding does not overlap with class token, so
            # add then concat.
            x = x + pos_embed
            if to_cat:
    ...
    def get_intermediate_layers(
        ...
        """
        # Take last n blocks if n is an int, or select by matching
        # indices when n is a sequence.
        ...
        if return_prefix_tokens:
            return tuple(zip(outputs, prefix_tokens, strict=True))
