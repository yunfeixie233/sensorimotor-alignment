...
def main():
    ...
    # Use the tokenizer from the policy processor -- same tokenizer that built input_ids
    ...
    print(f"  tokenizer type: {type(backbone_tokenizer).__name__}")
    ...
    for i in range(args.resume_from, num_samples):
        ...
        def backbone_post_hook(module, inputs, outputs):
            ...
            backbone_outputs_capture["image_mask"] = outputs["image_mask"].detach()
            backbone_outputs_capture["input_ids"] = inputs[0]["input_ids"].detach()
        ...
        image_mask = backbone_outputs_capture["image_mask"]
        input_ids = backbone_outputs_capture["input_ids"]
        ...
        # GR00T N1.6 builds its sequence via a private processor format.
        # The input_ids in the backbone do not contain raw task text tokens
        # in a searchable layout, so build_task_mask always returns all-zeros.
        # Use non-image, non-padding positions as the text-region mask instead:
        # text_mask = backbone_mask AND NOT image_mask.
        ...
        feat_txt = masked_mean_pool(backbone_features, text_mask).squeeze(0).cpu()
    ...
    elapsed = time.time() - t0
    print(f"\n=== GR00T N1.6 Unified Extraction Complete ===")
